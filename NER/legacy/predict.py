...
class Predict(object):
    ...
    def get_args(self):
        return torch.load(os.path.join(self.pred_config.model_dir, 'training_args.bin'))


    def load_model(self, args, device):
        # Check whether model exists
        if not os.path.exists(self.pred_config.model_dir):
    ...
    def read_input_file(self, tokenizer):
        ...
        lines = []
        texts = []
        with open(self.pred_config.input_file, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
